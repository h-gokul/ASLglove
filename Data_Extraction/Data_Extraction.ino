...
int c1, c2, c3,c4,c5,c6, index, mSum[26], mini[2] = {29, 29};
int flex[4] = {}, flex1[4] = {};
int nobend[4] = {};
int  flag, fullbend[4] = {};
float range[4];
...
void loop() {
  ...
  if (state_ == 1) {
    ...
    while (millis() - prevtime_ <= 3000) {
      ...
      Serial.print(yaw_); Serial.print(" "); Serial.print(pitch_); Serial.print(" "); Serial.println(roll_);
...
void Read() {
  ...
  c1 = digitalRead(7); c2 = digitalRead(6); c3 = digitalRead(5);c4 = digitalRead(4);c5 = digitalRead(3);c6 = digitalRead(2);
  // read Inertial Measurement Unit (IMU) data
  mpu6050.update();
  yaw_ = mpu6050.getAngleX();
  pitch_ = mpu6050.getAngleY();
  roll_ = mpu6050.getAngleZ();
  
  //  map the range of different flex sensor's data to a common 0-100 scale and constraint the values within this range
  for (int  i = 0; i <= 3; i++) {
    // constant 1.2 is chosen to allow a 20% tolerance for restricting outliers.
